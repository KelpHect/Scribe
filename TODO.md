...
- [x] Move initial AddOns scan off the startup critical path.
  ...
  - Completed: startup now configures the scanner path without parsing the AddOns tree, starts one explicit startup scan, `GetInstalledAddons` returns cached state without starting implicit scans, and diagnostics now expose cached-state-ready, scan-start, scan-ready, in-flight, and scan-error fields.
  - Verification: `startup_scan_test.go` proves `GetInstalledAddons` returns cached state without starting a scan and `RefreshInstalledAddons` performs an explicit scan.
- [x] Stop installed scan/refetch loops from exhausting memory.
  - Evidence: `installed:scan-complete` refetched installed state, while query handlers could start another scan and emit another completion event, creating a repeating scan/refetch loop that matched high allocation and goroutine diagnostics.
  - Acceptance criteria: query fetches do not start implicit scans; only user-visible startup/path/settings/manual refresh scans emit `installed:scan-complete`; manual refresh still scans on demand.
  - Completed: `GetInstalledAddons` is cache-only, `RefreshInstalledAddons` performs explicit scans, frontend manual refresh buttons use the explicit scan path, and scan-complete events are suppressed for query-triggered scan reasons such as matched/addon update/dependency queries.
  - Verification: `startup_scan_test.go` covers cache-only installed fetches, explicit refresh scans, and scan-complete reason filtering; a watchdog desktop launch stayed around 192 MB RSS instead of exceeding the 1 GB guard.
...
- [x] Optimize remote catalog cache writes and whole-catalog loads for unchanged and mostly unchanged refreshes.
  - Evidence: repeated full remote catalog saves used to delete and reinsert every addon/category row, so an unchanged 7k-addon fixture refresh measured hundreds of milliseconds on high-end hardware.
  - Acceptance criteria: store a normalized catalog hash, skip large writes when unchanged, keep a custom-binary snapshot cache for whole-catalog loads with JSON fallback for old caches, preserve row tables for compatibility, raw-upsert changed rows, delete vanished rows only, and benchmark unchanged, changed-one, initial, codec, and load paths.
  - Completed: remote catalog saves now compute a SHA-256 hash over normalized feed/addon/category rows; unchanged refreshes update only fetched metadata, old JSON snapshots upgrade to custom binary without rewriting compatibility rows, snapshot loads avoid thousands of row scans, changed saves write the versioned binary snapshot plus raw prepared upserts for changed compatibility rows, and stale compatibility rows are deleted by UID/category ID.
  - Verification: cache tests cover binary snapshot precedence, legacy JSON snapshot fallback, unchanged legacy snapshot upgrade, unchanged-save row-skip behavior, changed-row diff/deletion, order-insensitive hashing, schema invalidation cleanup, and SQLite PRAGMAs; `BenchmarkCachedCatalogLoad`, `BenchmarkRemoteCatalogSnapshotCodecs`, `BenchmarkSQLiteSaveRemoteCatalog`, `BenchmarkSQLiteSaveRemoteCatalogChangedOne`, and `BenchmarkSQLiteSaveRemoteCatalogInitial` report the separate cache paths.
...
- [x] Add a worker-backed catalog filtering spike if main-thread filtering still exceeds frame budget.
...
- [x] Run an alternate desktop-shell spike only if Wails itself is proven to be the bottleneck.
  ...
  - Completed: spike was intentionally not run or kept because no current profile proves Wails/WebKitGTK is the bottleneck; recent fixes targeted Scribe-owned startup, cache, filtering, progress, image, and install paths instead. `docs/desktop-stack-evaluation.md` records the alternate-shell decision and the exact trigger for revisiting.
  - Verification: the desktop stack evaluation keeps Tauri/Electron/custom shell work gated behind measured Wails-specific bottleneck evidence and an accepted migration plan.
...
- [x] Remote catalog cache is SQLite-backed and schema-versioned.
  - Evidence: `internal/esoui/cache.go` stores versioned custom-binary remote catalog snapshots with JSON fallback, compatibility addon/category rows, and cache metadata in `esoui_cache.db` under the historical `Scribe` config dir with `cacheSchemaVersion = "2"`.
