...
- [x] Optimize remote catalog cache writes and whole-catalog loads for unchanged and mostly unchanged refreshes.
  ...
  - Completed: remote catalog saves now compute a SHA-256 hash over normalized feed/addon/category rows; unchanged refreshes update only fetched metadata, old JSON snapshots upgrade to custom binary without rewriting compatibility rows, snapshot loads avoid thousands of row scans and string copies from immutable blob data, changed saves write the versioned binary snapshot plus raw prepared upserts for changed compatibility rows, and stale compatibility rows are deleted by UID/category ID.
