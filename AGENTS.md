...
- `tray.rs` — Windows tray icon: background thread with a message-only window, mpsc events drained GPUI-side, balloon on raised update counts while minimized. With background alerts enabled, closing the window minimizes to the tray; disabled, close quits normally.
...
- Shell: 228px left rail (sidebar tint + hairline) with the single Scribe wordmark, icon nav with accent selection and count badges, Settings pinned to the bottom. Content column: 36px caption strip, then a page header row (page title + subtitle + catalog health on the left, per-page actions on the right), then the filter row, then an inline status banner. Work surfaces are centered at 1200px (Settings: 760px reading column) with 28px gutters.
- The caption strip is Scribe-owned (no component `TitleBar`): a plain drag region with `WindowControlArea::Drag` keeps native move/maximize/system-menu behavior via HTCAPTION, and the Scribe Windows-control overlay above it holds 46x32 native `WindowControlArea` hit regions. Do not reintroduce the gpui-component `TitleBar` — its full-width drag hitbox always wins the non-client hit test over anything painted above it (paint-order iteration), which turned clicks on the Scribe controls and on overlay close buttons into window drags. All caption hit areas (drag + controls) are suspended via `window_chrome_active` whenever a modal overlay or context menu is open.
...
- Details breakpoint: on viewports ≥ 1400px dossiers open inline as a page-level view (back affordance + Escape, left flow with 200px screenshot rail, 300px facts/actions rail, full-width changelog); below that they keep the modal sheet. Both map to the same OverlayKind so focus, Escape, and keyboard guards are identical.
- Keyboard browsing: `↓`/`↑` (or `j`/`k`) move the active row, `Enter` opens details, `i` installs/updates, `/` focuses search, Escape blurs search — only when no overlay/menu/palette is open and the search input is unfocused. Find More search is fuzzy (app-side score over the core-filtered base, popularity tiebreak; never sort when the query is empty); Enter commits up to 8 recent searches shown when the input is focused and empty.
...
- Keep large catalogs virtualized with stable row geometry (72px catalog pitch) and bounded overscan; the Installed/Updates library virtualizes its flattened group model through `gpui::list` with a window-held `ListState` (splice on count change, never rebuild scroll state per frame).
