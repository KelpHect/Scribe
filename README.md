...
Release builds end up in `bin/`.

### Linux build packages

Linux Wails builds need GTK, WebKitGTK, a C/C++ compiler, `pkg-config`, and npm. Scribe keeps the Wails v3 Linux build on GTK3 for now, so use the `gtk3` tag.
...
Release tags are `vX.Y.Z` and must match `frontend/package.json`.

When GitHub Actions are available, the release workflow builds the platform matrix and publishes the staged assets it validates:

- `Scribe-windows-amd64.exe`
- `Scribe-linux-amd64`
- `Scribe-macos-universal.zip`
- `Scribe-windows-amd64-installer.exe` only when the NSIS build produces an installer

When releasing locally with `gh`, only publish artifacts that were built and smoke-tested locally. The current local fallback asset set is Windows amd64, Linux amd64, Fedora 44 Linux amd64, and `SHA256SUMS.txt`; macOS still requires a macOS runner or build host.
...
- Linux and Windows release helpers compress with UPX only when it is installed locally
- macOS assets are produced only by the macOS workflow/build host; when present they are ad-hoc signed, not notarized, so Gatekeeper may still warn
...
The Vite dev server binds to `127.0.0.1` by default through `WAILS_VITE_HOST` so the Wails asset proxy and frontend server use the same loopback address on Windows. Override it only when you need a different local bind host.
...
On Linux, local Wails builds need `-tags gtk3` after installing the GTK/WebKit packages above. Use `npm --prefix frontend run lint:check` for Oxlint verification. `npm --prefix frontend run format:check` checks Oxfmt-supported TypeScript, JavaScript, and CSS files; Svelte component formatting is still kept by focused edits, review, and `svelte-check`.

### generated files

`frontend/bindings/`, `frontend/dist/`, `bin/`, `.task/`, and generated platform icon outputs are produced by Wails or local build tools. Do not hand-edit them or treat them as source files.

If `frontend/bindings/` is missing or stale, run `wails3 task common:generate:bindings` or `wails3 build`. If `frontend/dist/` is missing, run `wails3 build`; root `go test ./...` needs those embedded assets to exist. From a clean checkout, `./scripts/verify.sh` does the recovery path for you.
...
- `frontend/bindings/` and `frontend/dist/` are generated by Wails
...
- set `SCRIBE_PPROF=1` to start the local pprof server on `localhost:6060`; `./scripts/profile-desktop.sh` captures CPU, heap, goroutine, and trace reports from a running app
- release builds can opt into Go PGO with `SCRIBE_PGO_PROFILE=/path/to/desktop.cpu.pprof ./scripts/build-release.sh ...`; use only representative desktop profiles
- release tagging is manual; the tag-release workflow reads the version from `frontend/package.json`, and local fallback releases should use the same `vX.Y.Z` convention
